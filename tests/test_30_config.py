...
# from src.core.advanced.config import split_field,get_files,get_file_info,build
...
# def test_build():
#     ops_map,slots,cranes,procs=build('test01')
#     assert ops_map[101].name=='上料'
#     assert len(slots)==3
#     assert str(slots[0]) =='[1] 上料 1|2'
#     assert len(cranes)==1
#     assert str(cranes[0])=='[1] H11 1 (1.0,1.0)'
#     assert len(procs)==3
#     assert str(procs[0]) =='[A] 上料 0[0,0]'
...
# import hydra
# @hydra.main(config_path="../conf", config_name="args", version_base="1.3")
