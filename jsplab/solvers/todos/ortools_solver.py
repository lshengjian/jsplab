...
def solve_instances(data_source:str='Mk',max_solve_time=120):
    """
    ...
    pf=Path(f'./or_solution/{data_source}')


    if not pf.exists():
        os.makedirs(f'./or_solution/{data_source}')
    dir=Path(__file__).parent.parent.parent
    data_list = load_data_from_files(dir/f'data/{data_source}')

    # save_direc = f'./or_solution/{data_source}'
    # if not os.path.exists(save_direc):
    #     os.makedirs(save_direc)

    #for data in data_list:
    for k in tqdm(range(len(data_list)), file=sys.stdout, desc="progress", colour='blue'):
        data=data_list[k]
        # data_name = data[1]
        ...
        # 
        jobs, num_machines = matrix_to_the_format_for_solving(data.num_tasks, data.op_times)
        solution, solveTime = fjsp_solver(jobs=jobs,
                                                num_machines=num_machines,
                                                time_limits=max_solve_time)
        #print(data.name,solution, solveTime)
        tqdm.write(f"Instance {data.name}, solution:{solution}, solveTime:{solveTime}, systemtime:{time.strftime('%m-%d %H:%M:%S')}")
...
def fjsp_solver(jobs, num_machines, time_limits):
    ...
    for job_id in all_jobs:
        ...
        for task_id in range(len(jobs[job_id])):
            ...
            print(
                f'  J{job_id+1}_{task_id+1}  M{machine+1}:{start_value}->{start_value+duration}')
