...
def make_workpiece(num_side=3): 
    ...
    img=np.zeros((TILE_SIZE,TILE_SIZE,NUM_CHANNEL),dtype=np.uint8)
...
def make_buff():
    #print('make buff')
    img=np.zeros((TILE_SIZE*2,TILE_SIZE,NUM_CHANNEL),dtype=np.uint8)
...
def get_progress_bar(p:int=100):
    p=np.clip(p,1,100)
    n=int(TILE_SIZE*(100-p)/100)
    img=np.ones((TILE_SIZE//4,n,NUM_CHANNEL),dtype=np.uint8)*255
    return img
