...
from .factory import JobShopFactory
from ...agents.heuristic.heuristic_agent import HeuristicSelectionAgent
from ...envs.old.env_tetris_scheduling import Env
...
def generate_instances_from_config(config: dict, print_info: bool = True) -> List[List[Task]]:
    ...
    """
    # Generate instances
    instances = JobShopFactory.generate_instances(**config)
...
def main(config_file_name=None, external_config=None):
    ...
    JobShopFactory.set_deadlines_to_max_deadline_per_job(instance_list, current_config.get('num_jobs', None))
