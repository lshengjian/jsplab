...
class OrToolSolver:
    def __init__(self,info: Instance,agv_up_time=2,agv_down_time=2,scale=2):
        ...
        self.num_machines = len(self.offsets)
        self.jobs = convert2fjsp_data(info.tasks)
        self.agv_start = info.first_crane_index
        self.agv_num = self.num_machines - self.agv_start
        self.max_steps=get_max_steps(info,agv_up_time,agv_down_time,scale)
    ...
    def init_agvs_pos(self):
        model=self.model
        agv_steps: List[cp_model.IntVar] = [None]*self.agv_num*self.max_steps #把多维数组压缩为一维数组
        for agv in range(self.agv_num):
            agv_steps[agv*self.max_steps]=model.NewConstant(self.offsets[self.agv_start + agv])
            for t in range(1, self.max_steps):
                idx=agv*self.max_steps+t
        ...
        for agv in range(self.agv_num - 1):
            for t in range(self.max_steps):
                idx1=agv*self.max_steps+t
                idx2=(agv+1)*self.max_steps+t
                model.Add(agv_steps[idx1] + 2 <= agv_steps[idx2])  
        self.agv_steps= agv_steps 
        self.offsets_var=[model.NewConstant(self.offsets[i]) for i in range(self.num_machines)]
        # assert len(offsets_var)==self.machines_count

    def optimize(self,max_solve_time=20,pause_time=0.16):
        model=self.model
        all_machines = range(self.num_machines)
        ...
        for job_id, job in enumerate(self.jobs):
            for task_id, task in enumerate(job):
                ...
                start_var = model.NewIntVar(0, self.max_steps , f"start{suffix}")
                end_var = model.NewIntVar(0, self.max_steps, f"end{suffix}")
                interval_var = model.NewIntervalVar(
                    start_var, duration_var, end_var, f"interval{suffix}"
                )
                machine_var = model.NewIntVar(
                    0, self.num_machines-1, f"machine{suffix}"
                )
                # add to all_tasks
                ...
                for alt_id, alt in enumerate(task):
                    ...
                    alt_start = model.NewIntVar(0, self.max_steps, f"start_{alternative_suffix}")
                    alt_end = model.NewIntVar(0, self.max_steps, f"end_{alternative_suffix}")
        ...
        obj_var = model.NewIntVar(0, self.max_steps, "makespan")
    ...
    def check_agv(self, machine_usage,all_tasks, job_id, task_id, alt, \
        ...
        rt=alt_duration
        time_step=model.NewIntVar(0, self.max_steps*self.agv_num,'')
        ...
        if task_id%2==1:# AGV开始作业位置是前一加工机器的位置
            ...
            model.Add(time_step == agv_index*self.max_steps+alt_start).OnlyEnforceIf(machine_usage)
            ...
            model.Add(time_step == agv_index*self.max_steps+agv_task.end).OnlyEnforceIf(machine_usage)
            model.AddElement(time_step,self.agv_steps,agv_x)
            ...
            for i in range(1,1+self.agv_up_time):
                ...
                tp = model.NewIntVar(0, self.max_steps*self.agv_num, "")
                model.Add(tp == agv_index*self.max_steps+agv_task.start+i).OnlyEnforceIf(machine_usage)
            ...
            for i in range(1,1+self.agv_down_time):
                p = model.NewIntVar(min_x, max_x, "")
                tp = model.NewIntVar(0, self.max_steps*self.agv_num, "")
                model.Add(tp == agv_index*self.max_steps+agv_task.end-i).OnlyEnforceIf(machine_usage)
                model.AddElement(tp,self.agv_steps,p)
    ...
    def get_agvs_pos(self,solver,data)->Tuple[Dict[int,List[int]],Dict[int,List[int]]]:
        ...
        for i in range(self.agv_num):
            for t in range(n):
                x=solver.Value(self.agv_steps[i*self.max_steps+t])
                rt[i].append(x)
        rt2={}
        for i in range(self.agv_num):
