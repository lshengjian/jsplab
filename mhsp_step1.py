...
for i,task in   enumerate(job):
    for alt in task:
        hoist,from_tank,to_tank,move_time,op_time = alt
        assert hoist in [0,1]
        assert to_tank in steps[i]
        assert move_time==abs(from_tank-to_tank)+up_time+down_time
        break
num_hoists = 2
all_hoists = range(num_hoists)
# num_tanks = 3
# all_tanks = range(num_tanks)
# Computes horizon dynamically as the sum of all durations.
horizon = sum(task[0].move_time+task[0].op_time  for task in job) #18+38+20+8=84
# 定义时间范围
print(horizon )
...
for task_id, task in enumerate(job):
    ...
    #     dx=-1 if to_tank>from_tank else 1
    #     for i in range(1,1+move_time):
    #         x=model.new_int_var(0, 9, '')
    #         temp=model.new_int_var(0, horizon, '')
    #         model.add(temp==end_var+i)
    #         model.add_element(temp,hoists_pos[hoist],x)
    #         model.add(x==to_tank+dx)    
    interval_var = model.new_interval_var(
        start_var, move_time, end_var, "interval" + suffix
    )
    all_tasks[task_id,0] = task_type(
...
for task_id in range(len(job) - 1):
    model.add(
        all_tasks[task_id + 1,0].start == all_tasks[task_id ,1].end
    )
...
model.add_max_equality(
    obj_var,
    [all_tasks[task_id, 1].end for task_id, task in enumerate(job)],
)
...
if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
    ...
    for task_id, task in enumerate(job):
        ...
        assigned_hoists[hoist].append(
            assigned_task_type(
                start=solver.value(all_tasks[task_id,0].start),
                job=0,
                index=task_id,
                duration=move_time
            )
        ...
        assigned_tanks[to_tank].append(
            assigned_task_type(
                start=solver.value(all_tasks[task_id,1].start),
                job=0,
                index=task_id,
                duration=op_time
            )
    ...
    for machine in all_hoists:
        ...
        for assigned_task in assigned_hoists[machine]:
            name = f"task_{assigned_task.index}"
