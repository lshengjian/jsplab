...
n = 5  # 化学槽数量
m = 3  # 起重机数量
w = [0, 3, 5, 7, 9, 0]  # 槽的位置，包括加载/卸载站（w[0]和w[n+1]）
...
l = [max(1, min(m, m - int((lambda_ - max(w[i], w[i + 1])) / d_value))) if m - int((lambda_ - max(w[i], w[i + 1])) / d_value) > 0 else 1 for i in range(n)]
u = [min(m, max(1, int((1 + min(w[i], w[i + 1]) - 0) / d_value))) if int((1 + min(w[i], w[i + 1]) - 0) / d_value) > 0 else m for i in range(n)]
print("n:", n)
for i in range(n):
    print("i:", i)
    print("l[i]:", l[i])
...
t = [model.NewIntVar(0, 1000, 't_{}'.format(i)) for i in range(n + 1)]
z = [[model.NewIntVar(0, 1, 'z_{}_{}'.format(i, k)) for k in range(l[i], u[i] + 1)] if l[i] <= u[i] else [model.NewIntVar(0, 1, 'z_{}_{}'.format(i, k)) for k in range(0, 1)] for i in range(n + 1)]
