...
mutation_rate=0.1 # default value is 0.1
...
for n in range(num_iteration):
    ...
    for m in range(int(population_size/2)):
        ...
        if crossover_rate>=crossover_prob:
            ...
            child_1=[-1 for i in range(num_job)]
            child_2=[-1 for i in range(num_job)]
            ...
            for i in range(num_job-fix_num):
                child_1[child_1.index(-1)]=c1[i]
                child_2[child_2.index(-1)]=c2[i]
...
print(f"optimal value:{Tbest:.1f}")
print(f"average tardiness:{(Tbest/num_job):.1f}")
print(f"number of tardy:{num_tardy}")
print(f'the elapsed time:{str(time.time() - start_time)}')
